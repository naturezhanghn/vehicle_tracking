...
namespace mot {
    ...
    T bbox_iou(const Rect<T> &box1,const Rect<T> &box2) {
        ...
        T b1_x1 = std::max(box1.x, (T)0.);
        T b1_y1 = std::max(box1.y, (T)0.);
        T b1_x2 = std::max(box1.x + box1.w,(T)0.);
        T b1_y2 = std::max(box1.y + box1.h,(T)0.);

        T b2_x1 = std::max(box2.x,(T)0.);
        T b2_x2 = std::max(box2.x + box2.w,(T)0.);
        T b2_y1 = std::max(box2.y,(T)0.);
        T b2_y2 = std::max(box2.y + box2.h,(T)0.);
        ...
        T inter_area;
        inter_area = 
            std::max((inter_rect_x2 - inter_rect_x1),(T)0.) * std::max((inter_rect_y2 - inter_rect_y1),(T)0.);
